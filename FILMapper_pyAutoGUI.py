...
def run_automation(overlay):
    ...
    if choice == "1":
        ...
        steps = [
            ...
            (None, "Click on the file explorer and wait for file to load", 2),
            (None, "Click home icon in file explorer", None),
        ]
        ...
        for step in steps:
            ...
            log(f"Prompt: {prompt}", True)
            overlay.update_text_signal.emit(f"Terminal: {prompt}")
            print(f"\nPrompt: {prompt}")

            # If there's an extra parameter (keyboard key or wait time)
            if extra:
                extra_param = extra[0]
                if isinstance(extra_param, str):
                    keyboard.wait(extra_param)
                elif isinstance(extra_param, (int, float)):
                    time.sleep(extra_param)
